...
首版采用全文读取和全文替换。性能门槛以发布优化构建为准：5 MiB 文档的一次全文替换往返（发送完整正文并返回完整文档快照）在本机的 P95 延迟超过 100 ms 时，才评估增量 patch；不提前引入 OT/CRDT。
...
- 文本 patch：典型文档大于 5 MiB，或发布优化构建下 5 MiB 全文替换往返的本机 P95 超过 100 ms。
