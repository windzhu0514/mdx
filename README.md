...
Mora 墨笺是一款本地优先的桌面笔记软件，支持标准 Markdown 文件与单文件 MDXNote 笔记。既可以直接编辑 `.md`，也可以将正文、图片和附件封装在一个 `.mdx` 文件中，便于独立保存和传递。

> **当前源码版本：** `0.1.3`（发布准备中）。已公开版本及安装包以 [GitHub Releases](https://github.com/windzhu0514/mdx/releases) 为准。

当前公开测试版本：[Mora 0.1.3](https://github.com/windzhu0514/mdx/releases/tag/app-v0.1.3)。预发布版本不会推送到默认自动更新通道。
...
- **Markdown（`.md` / `.markdown`）**：直接打开、编辑并保存原文件，图片和附件保留为外部资源。源码保留完整 frontmatter；所见即所得和预览仅显示正文。
- **Mora MDX（`.mdx`）**：正文、图片和附件封装在同一个 MDXNote 文件中。

在“文件”菜单选择“新建 Markdown”或“新建 MDX”；“保存”及 `Ctrl+S` 沿用当前格式。“另存为 Markdown”与“另存为 MDX”用于转换，原文件保留。窗口顶部会标明当前保存格式。

跨目录或跨格式另存为会复制本地资源并修正引用。导出的 Markdown 资源通常位于文档旁的 `<文件名>_files/`，分享时需连同资源目录一起携带；网络链接保持链接，缺失资源会提示处理。普通 Markdown 不使用额外的元数据文件。

两种格式共用所见即所得、源码、源码与预览视图，均支持草稿、外部修改检测与保存冲突保护。内置历史版本和包内附件重命名、删除适用于 MDX；Markdown 附件可添加、插入引用、打开与导出，外置文件的重命名和删除在文件夹中管理，避免删除其他笔记共用的资源。

## Mora 重点解决的事情

- **笔记与资源不分家：** 粘贴图片、选择附件或拖入文件后，资源随笔记一同保存，不需要手动整理配套目录。
- **文件归用户管理：** 笔记保存在用户选择的位置，可以使用现有的文件夹、移动硬盘和备份工具管理。
- **支持两种保存格式：** `.md`、`.markdown` 可直接原格式保存；也可与 `.mdx` 相互另存转换。
...
笔记按所选格式保存为 `.md` / `.markdown` 与外置资源，或封装在 `.mdx` 文件中。恢复草稿、最近打开记录和工作区全文搜索索引保存在 Mora 的应用数据目录中。索引刷新由打开工作区、窗口重新获得焦点或用户手动刷新触发，不会持续监听磁盘。
...
Mora 也会监视已打开 `.md`、`.markdown`、`.mdx` 的磁盘变化：文档干净时自动重新载入，存在未保存编辑时保留内存内容并进入冲突处理。关闭 Agent 接入会移除发现记录、断开 watcher，并拒绝后续请求。
