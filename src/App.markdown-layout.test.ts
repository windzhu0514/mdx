...
describe("App 编辑器视图", () => {
    ...
    it("keeps the titlebar mode switch compact", () => {
        expect(styleCss).toMatch(
            /\.mode-switch\.compact\s*\{[^}]*height:\s*28px[^}]*padding:\s*2px/su,
        );
        expect(styleCss).toMatch(
            /\.mode-switch\.compact button\s*\{[^}]*padding:\s*4px 10px[^}]*font-size:\s*12px/su,
        );
    });
