...
5. **Command focus handoff** — closed. Palette close restores its invoker by default through an explicit parent prop. `App` suppresses restoration only after the executed action has actually opened a blocking modal; ordinary commands and History's unsaved-document early return restore the invoker. Settings and History dialogs focus their own panel on open. Covered by `CommandPalette.test.ts`, `App.web.test.ts`, and `panelAccessibility.test.ts`.
...
- Final re-review found that blanket suppression still left ordinary and early-return commands focused on `body`. Regression tests were added first for an ordinary palette command and unsaved History; the parent now decides focus restoration from the actual post-action modal state and passes that decision explicitly to the palette.
